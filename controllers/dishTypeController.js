...
function viewAll (req,res){
    ...
    DishType.find().sort('type').paginate(page, types_per_page, (err, types, total)=>{
        if(err) return res.status(500).send({message: 'Hubo un error en la petición'});
        return res.status(200).send({
            message     :   'Lista de tipos de platillos',
            types    :   types,
            total       :   total,
...
function viewType (req,res){
    ...
    DishType.findById(typeToView).exec((err,type)=>{
        if(err) return res.status(500).send({message: 'Hubo un error en la petición'});
        if(!type) return res.status(404).send({message: 'El tipo de producto no existe'});
        return res.status(200).send({
            message     :   'Tipo de platillo encontrado',
            type     :   type
        });
    });
}
function updateType (req,res){
    ...
    DishType.findByIdAndUpdate(typeToUpdate,updateInfo,{new: true}, (err, updatedType)=>{
        if(err) return res.status(500).send({message: 'Hubo un error en la petición'});
        if(!updatedType) return res.status(304).send({message: 'No se pudo actualizar el tipo de producto'});
        return res.status(200).send({
            message     :   'Tipo de platillo actualizado',
            type     :   updatedType
        });
    });
}
...
function createType (req,res){
    ...
    if(typeParams.type){
            let newType = new DishType({
                type        : typeParams.type,
                enabled     :   true
            });
            DishType.find({type: newType.type}).exec((err, foundedType) => {
                if (err) return res.status(500).send({ message: 'Hubo un error en la petición' });
                ...
                newType.save((err, type) => {
                    ...
                    if (!type) return res.status(201).send({
                        message     : 'No se ha registrado el tipo de platillo',
                        type    : type
                    });
                    return res.status(201).send({
                        message     : 'Tipo de platillo registrado con éxito',
                        type     : type
                    });
                });
...
function deleteType (req,res){
    ...
    DishType.findById(typeToDelete,(err, type) => {
        if(err) return res.status(500).send({message: 'Hubo un error en la petición'});
        if(!type) return res.status(404).send({message: 'No existe el tipo de producto'});
        DishType.findByIdAndRemove({'_id':typeToDelete}).exec().then((removed) => {
            if(!removed) return res.status(401).send({message: 'El tipo de platillo no ha sido eliminado'});
            return res.status(200).send({message: 'Tipo de platillo eliminado con éxito'});
        }).catch((err) => {
            return err;
        });
    });
