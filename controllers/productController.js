...
function viewAll (req,res){
    ...
    Product.find().populate('category').sort(order).paginate(page, pdcts_per_page, (err, products, total)=>{
...
function viewProduct (req,res){
    ...
    Product.findById(productToView).populate('category').exec((err,product)=>{
        if(err) return res.status(500).send({message: 'Hubo un error en la petición'});
...
function createProduct (req,res){
    ...
    if(productParams.name && productParams.category && productParams.description && productParams.price){
